...
public class Tests_3_1_2 {
	...
	@Test
	public void testSuggestedProductAddedToReservation() {
		Product tProd = new ProductBuilder()
				.unavailable()
				.withName("test product")
				.withPrice(new Money(123))
		...
		Mockito.when(suSe.suggestEquivalent(tProd, testC)).thenReturn(sProd);
		testHandler.handle(testCommand);
		assertThat(tRes.contains(sProd), is(equalTo(true)));
		assertThat(tRes.contains(tProd), is(equalTo(false)));
	}
	
	@Test (expected = DomainOperationException.class)
	public void testSuggestedProductNotAddedToClosedReservation() {
		Product tProd = new ProductBuilder()
				.unavailable()
				.withName("test product")
				.withPrice(new Money(123))
